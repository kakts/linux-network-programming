/**
 * 9.8 UDP/IPブロードキャスト用クライアント
 *
 * 同一ネットワークセグメントの全てのホストに通知される
 * u-client.cのmain関数でソケットにブロードキャストの設定を許可するのみ
 * 
 * 確認方法
 * 複数サーバを起動して、ブロードキャストアドレスを指定して送信する
 */
 ...
 * 
 * ソケットに対し、ブロードキャストの許可をsetsockopt()により行う
 */
